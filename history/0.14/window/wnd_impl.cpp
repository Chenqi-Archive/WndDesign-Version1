...
WNDDESIGN_API Alloc Wnd* Wnd::Create(Ref WndBase* parent, const WndStyle& style, Handler handler) {
	_Wnd_Impl* wnd = new _Wnd_Impl(new WndStyle(style), handler);
...
_Wnd_Impl::_Wnd_Impl(Alloc WndStyle* style, Handler handler) :
	_WndBase_Impl(style, handler),
	_base_layer(this) {
}

_Wnd_Impl::_Wnd_Impl(Alloc WndStyle* style, Handler handler, ScrollWndStyle::RegionStyle entire_region) :
	_WndBase_Impl(style, handler),
...
void _Wnd_Impl::Composite(Rect region_to_update) const {
	...
	Figure* border = new Rectangle(GetStyle().border._width, GetStyle().border._color, color_transparent);
	figure_queue.Push(border, Rect(point_zero, GetSize()));

	DrawFigureQueue(figure_queue, region_to_update);
}

void _Wnd_Impl::LayerUpdated(const Ref Layer* layer, Rect region) const {
...
bool _Wnd_Impl::ShouldAllocateNewLayer(Ref _WndBase_Impl* child, Rect region) {
	...
	for (const auto& it : _child_wnds) {
		...
		if (!it->RegionOnParent().Intersect(region).IsEmpty()) {
			if (MayRedraw(it->GetStyle())) { return true; }
...
bool _Wnd_Impl::AddChild(Ref WndBase* child_wnd) {
	...
	Rect region = CalculateActualRegion(style, GetSize());
...
bool _Wnd_Impl::RemoveChild(Ref WndBase* child_wnd) {
	...
	// Clear the child window's parent.
...
void _Wnd_Impl::MoveChild(Ref WndBase* child_wnd) {
	...
	Rect union_region = new_region.Union(old_region);

	if (child->GetLayer() == &_base_layer) {
		// The child window is on the base layer.
		// The base layer will redraw and automatically call the parent to composite.
		_base_layer.MoveWnd(child, new_region);
		_base_layer.RegionUpdated(_base_layer.ConvertToLayerRegion(union_region), false);
	} else {
