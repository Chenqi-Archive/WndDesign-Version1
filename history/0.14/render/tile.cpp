...
#undef DrawText
#endif
void Tile::DrawText(Rect region, const wstring& text, const Ref TextBoxStyle& style) {
	ID2D1RenderTarget* target = _target->GetTarget();
	TextLayout layout = CreateTextLayout(text, region.size, style);
	ID2D1SolidColorBrush* brush = _tile_cache->GetResourceManager()->UseSolidColorBrush();
	brush->SetColor(Color2COLOR(style.font._color));
	target->DrawTextLayout(Point2POINT(region.point), layout.Get(), brush,
