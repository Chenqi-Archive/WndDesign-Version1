...
public:
	Layer(Ref _WndBase_Impl* parent, Rect composite_region, bool is_redirected,   // Relative to parent window.
		  Rect accessible_region, Point current_point,				// Relative to the layer itself.
		  Color32 background, WndStyle::RenderStyle style = WndStyle::RenderStyle());
...
public:
	...
	const Rect ConvertToLayerRegion(Rect region_on_parent) const {
		return { ConvertToLayerPoint(region_on_parent.point), region_on_parent.size };
	}
...
private:
	...
	virtual void RefreshChildVisibleRegion(Rect visible_region) const pure;
...
class SingleWndLayer : public Layer {
	...
	///////////////////////////////////////////////////////////
public:
	SingleWndLayer(Ref _WndBase_Impl* parent, Rect composite_region, 
...
private:
	...
	void RefreshChildVisibleRegion(Rect visible_region) const override;
	// Only called by RegionUpdated.
...
class MultipleWndLayer : public Layer {
	...
	///////////////////////////////////////////////////////////
public:
	// For normal windows. The parent size is not sure at construction.
	MultipleWndLayer(Ref _WndBase_Impl* parent);
	
	// For scrollable windows.
	MultipleWndLayer(Ref _WndBase_Impl* parent, ScrollWndStyle::RegionStyle entire_region);
...
public:
	...
	// The parent window's size has changed, reset the size. The composition point is always point_zero.
