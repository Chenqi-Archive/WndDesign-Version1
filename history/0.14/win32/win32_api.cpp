...
extern void SystemTimerHandler(HANDLE timer);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_PAINT: {
            ...
            layer->DrawRegion(RECT2Rect(ps.rcPaint));
            EndPaint(hWnd, &ps);
        }break;
    case WM_ERASEBKGND: break;  // Do nothing, just process the message.
...
void TimerProc(HWND Arg1, UINT Arg2, UINT_PTR Arg3, DWORD Arg4) {
    SystemTimerHandler(reinterpret_cast<HANDLE>(Arg3));
}
...
HANDLE SetTimer(HANDLE timer, uint period) {
    return reinterpret_cast<HANDLE>(::SetTimer(NULL, reinterpret_cast<UINT_PTR>(timer), period, TimerProc));
}
