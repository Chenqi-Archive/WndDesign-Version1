...
class WindowBorderResizer {
public:
	WindowBorderResizer(ObjectBase& parent) :
		_parent(parent),
...
private:
	enum class BorderPosition { None, Left, Top, Right, Bottom, LeftTop, RightTop, LeftBottom, RightBottom };
...
private:
	inline BorderPosition PointToBorderPosition(Size window_size, uint border_width, Point point) {
...
public:
	...
	bool TrackMsg(Msg msg, Para para) {
		...
		if (msg == Msg::ChildHit) {
			auto& child_hit_msg = GetMouseMsgChildHit(para);
			msg = child_hit_msg.original_msg;
		}
