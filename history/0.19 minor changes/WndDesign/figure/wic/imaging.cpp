...
ImageResource::ImageResource(const wstring& file_name) :
	_wic_image(WicFactory::Load(file_name)), 
	_wic_bitmap(nullptr),
...
ImageResource::ImageResource(void* file_address, size_t file_size) :
	_wic_image(WicFactory::Load(file_address, file_size)), 
	_wic_bitmap(nullptr),
	_bitmap_data(nullptr),
...
Color ImageResource::GetColor(Point point) {
	...
	if (_bitmap_data == nullptr) {
		WICRect lock_region = { 0, 0, static_cast<int>(size.width), static_cast<int>(size.height) };
...
void ImageResource::CreateD2DBitmap(Ref<TargetResourceManager*> resource_manager) {
	if (IsEmpty()) { ExceptionDialog(); return; }
	if (HasD2DBitmap()) { return; }

	if (_wic_bitmap == nullptr) {
		auto hr = resource_manager->GetCompatableTarget()->CreateBitmapFromWicBitmap(_wic_image, &_d2d_bitmap);
		if (FAILED(hr)) { Error(); return; }
	} else {
		auto hr = resource_manager->GetCompatableTarget()->CreateBitmapFromWicBitmap(_wic_bitmap, &_d2d_bitmap);
		if (FAILED(hr)) { Error(); return; }
	}
	_resource_manager = resource_manager;
	_resource_manager->InsertAllocatedImage(this);
}

void ImageResource::DestroyD2DBitmap() {
	if (HasD2DBitmap()) {
		_resource_manager->RemoveImage(this);
	}
}

void ImageResource::ClearD2DResource() {
	SafeRelease(&_d2d_bitmap);
	_resource_manager = nullptr;
}

ID2D1Bitmap& ImageResource::GetD2DBitmap() const {
	if (!HasD2DBitmap()) { Error(); }
	return *_d2d_bitmap;
}
