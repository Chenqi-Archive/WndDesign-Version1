...
public:
	...
	Ref<_WndBase_Impl*> GetParent() const {
		if (_layer == nullptr) { return nullptr; }
		return _layer->GetParent();
	}

	const Point ConvertToLayerPoint(Point point) const {
		if (_layer == nullptr) { return point; }
		return point + (_region.point - point_zero);
	}
	...
	const Point ConvertToDesktopPoint(Point point) const {
		if (_layer == nullptr) { return point; }
		return _layer->ConvertToDesktopPoint(point + (_region.point - point_zero));
	}
