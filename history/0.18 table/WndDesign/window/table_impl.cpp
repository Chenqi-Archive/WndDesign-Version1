...
WNDDESIGN_API unique_ptr<ITable, IWndBase::Deleter>
...
vector<_Table_Impl::Row>::iterator _Table_Impl::AdjustRowHeightToColumnWidthChange() {
	...
	for (int index_row = static_cast<int>(_rows.size()) - 1; index_row >= 0; --index_row) {
		...
		required_height = CalculateRowHeight(GetStyle(), GetSize().height, ValueTag(required_height));
...
vector<_Table_Impl::Column>::iterator _Table_Impl::AdjustColumnWidthToRowHeightChange() {
	...
	for (int index_column = static_cast<int>(_columns.size()) - 1; index_column >= 0; --index_column) {
		...
		required_width = CalculateColumnWidth(GetStyle(), GetSize().height, ValueTag(required_width));
...
void _Table_Impl::LayerSizeUpdated(Size unchanged_region) {
	...
	if (unchanged_region.width < size.width) {
		RegionUpdated(Rect(static_cast<int>(unchanged_region.width), 0, size.width - unchanged_region.width, unchanged_region.height));
	}
	if (unchanged_region.height < size.height) {
		RegionUpdated(Rect(0, static_cast<int>(unchanged_region.height), size.width, size.height - unchanged_region.height));
	}
}
...
void _Table_Impl::SizeChanged() {
	...
	if (GetStyle().auto_resize.NotAutoResize()) {
...
void _Table_Impl::Composite(Rect region_to_update) const {
	...
	auto style = GetStyle();
	figure_queue.Push(
		RoundedRectangle::Create(style.border._radius, style.border._width, style.border._color, background_transparent),
