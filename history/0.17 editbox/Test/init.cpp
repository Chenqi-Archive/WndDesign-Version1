...
IWnd* main_frame;
___ IScrollWnd* main_space;
___ ___ Button* floating_button;
Table* file_menu_table;
___ IWnd* save_file;
___ ___ IWndBase* saving_state;
MessageBox* about_dialog;
MessageBox* confirm_dialog;

bool main_space_handler(IWndBase* wnd, Msg msg, Para para) {
	if (msg == Msg::LeftDown) {
...
bool floating_button_handler(IWndBase* wnd, Msg msg, Para para) {
...
bool menu_bar_handler(IWndBase* wnd, Msg msg, Para para) {
...
bool file_menu_handler(IWndBase* wnd, Msg msg, Para para) {
	if (msg == Msg::LeftUp) {
...
bool close_button_handler(IWndBase* wnd, Msg msg, Para para) {
	if (msg == Msg::LeftUp) {
...
bool open_file_handler(IWndBase* wnd, Msg msg, Para para) {
	if (msg == Msg::LeftUp) {
		// open file dialog.
	}
	return true;
...
bool save_file_handler(IWndBase* wnd, Msg msg, Para para) {
	if (msg == Msg::LeftUp) {
...
bool saving_state_handler(IWndBase* wnd, Msg msg, Para para) {
	// Display a gif animation. When finished, remove itself from its parent.

	return true;
}

bool about_handler(IWndBase* wnd, Msg msg, Para para) {
	if (msg == Msg::LeftUp) {
...
bool about_dialog_handler(IWndBase* wnd, Msg msg, Para para) {
	if (msg == Msg::LeftUp) {
...
bool confirm_dialog_handler(IWndBase* wnd, Msg msg, Para para) {
	if (msg == Msg::LeftUp) {
...
void Init() {
	...
	main_frame = IWnd::Create(desktop, main_frame_style, nullptr); 
	{
		IWnd* menu_bar = IWnd::Create(main_frame, menu_bar_style, menu_bar_handler); 
		{
			...
			ITextBox* title = ITextBox::Create(menu_bar, title_style, nullptr);
		}
		main_space = IScrollWnd::Create(main_frame, &main_space_style, main_space_handler);
		{
	...
	file_menu_table = Table::Create(nullptr, file_menu_table_style);
	{
		...
		save_file = IWnd::Create(nullptr, save_file_style, nullptr);
		{
			Button* save = Button::Create(save_file, file_menu_table_button_style, save_file_handler, L"Save");
			saving_state = IWnd::Create(save_file, save_file_style, nullptr);
		}
