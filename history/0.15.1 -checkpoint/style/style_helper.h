...
inline Rect CalculateActualRegion(const WndStyle& style, Rect parent_region, Size& size_min, Size& size_max) {
	...
	region.point.x = position._left.AsSigned() + parent_region.point.x;
	region.point.y = position._top.AsSigned() + parent_region.point.y;

	size_min.width = size._min.width.AsUnsigned();
	size_min.height = size._min.height.AsUnsigned();
	size_max.width = size._max.width.AsUnsigned();
	size_max.height = size._max.height.AsUnsigned();

	return region;
}

// For normal Wnd and Desktop.
inline Rect CalculateActualRegion(const WndStyle& style, Size parent_size, Size& size_min , Size& size_max) {
	return CalculateActualRegion(style, Rect(point_zero, parent_size), size_min, size_max);
}
