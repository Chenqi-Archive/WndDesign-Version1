...
constexpr Size tile_size_max_fixed = max_tile_size;
...
TileCache::TileCache(Ref<TargetResourceManager*> resource_manager, Color background,
...
Tile& TileCache::WriteTile(TileID tile_id) {
	...
	if (it == _cache.end()) {
		...
		_cache.emplace(std::piecewise_construct,
					   std::make_tuple(TileIDToLongLong(tile_id)),
					   std::make_tuple(this, GetTileOrigin(tile_id, _tile_size), _tile_size)).first;
...
void TileCache::DrawBackground(Rect region) {
	...
	for (TileRangeIterator it(RegionToOverlappedTileRange(region, _tile_size)); !it.Finished(); ++it) {
		...
		if (region_to_draw.Contains(Rect(point_zero, _tile_size))) {
			...
			if (tile.BeginDraw(region_to_draw)) {
				_active_tiles.push_back(Ref<Tile*>(&tile));
			}
...
void TileCache::DrawOn(Point point, uchar opacity, Tile& target_tile, Rect region) const {
	...
	for (; !it.Finished(); ++it) {
		TileID tile_id = it.Item();
		Point tile_origin = GetTileOrigin(tile_id, _tile_size);
		Rect rect_on_tile = (my_region - (tile_origin - point_zero)).Intersect(Rect(point_zero, _tile_size));
