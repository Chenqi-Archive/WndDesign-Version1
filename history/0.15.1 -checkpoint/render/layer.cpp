...
Layer::Layer(_WndBase_Impl& parent, Rect composite_region, bool is_redirected,
			 Rect accessible_region, Point current_point, Color background, WndStyle::RenderStyle style):
	_parent(parent),
...
SingleWndLayer::SingleWndLayer(_WndBase_Impl& parent, Rect composite_region, Ref<_WndBase_Impl*> wnd,
	...
	_padding(padding) {
	_wnd->SetCanvas(Canvas(this, Rect(point_zero, composite_region.size)));
}
...
void SingleWndLayer::SetCompositeRegion(const Rect& composite_region) {
	...
	if (size_changed) {
		...
		_wnd->SetCanvas(Canvas(this, Rect(point_zero, _composite_region.size )));
	} else {
...
void MultipleWndLayer::RedrawChild(Rect region_to_update) const {
	...
	if (_is_redirected) {
		...
		figure_queue.Push(new SolidColor(_parent.GetStyle().background), region_to_update);
...
void MultipleWndLayer::ResetChildRegion() {
	...
	for (auto& it : _wnds) {
		...
		Rect region = CalculateActualRegion(it.wnd->GetStyle(), accessible_region, size_min, size_max);
		it.wnd->SetSizeStyle(size_min, size_max);
