...
void _Wnd_Impl::SizeChanged() {
	...
	for (auto& child : _child_wnds) {
		...
		Rect new_region = CalculateActualRegion(style, GetSize(), size_min, size_max);
...
bool _Wnd_Impl::AddChild(Ref<IWndBase*> child_wnd) {
	...
	Size size_min, size_max;
	Rect region = CalculateActualRegion(style, GetSize(), size_min, size_max);
	child->SetSizeStyle(size_min, size_max);
...
bool _Wnd_Impl::MoveChild(Ref<IWndBase*> child_wnd) {
	Ref<_WndBase_Impl*> child = dynamic_cast<Ref<_WndBase_Impl*>>(child_wnd);
	...
	Rect old_region = child->RegionOnParent();  // Relative to myself.
	...
	Rect new_region = CalculateActualRegion(style, GetSize(), size_min, size_max);
	child->SetSizeStyle(size_min, size_max);
	if (new_region == old_region) { return false; } // Relative to the base_layer if on base_layer.
	...
	if (child->GetLayer() == &_base_layer) {
		...
		union_region = _base_layer.ConvertToParentRegion(new_region).Union(old_region);
		...
		const_cast<SingleWndLayer*>(static_cast<const SingleWndLayer*>(child->GetLayer()))
			->SetCompositeRegion(new_region);
		union_region = new_region.Union(old_region);
	}
...
void _Wnd_Impl::SetChildRenderStyle(Ref<IWndBase*> child_wnd) {
	Ref<_WndBase_Impl*> child = dynamic_cast<Ref<_WndBase_Impl*>>(child_wnd);
