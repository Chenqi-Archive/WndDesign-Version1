...
void OnPaint() {
	...
	// Draw caret if not selected.
		...
		target->FillRectangle(Rect2RECT(caret_region), brush);
	}
...
extern void SetCaret(Rect caret_region);
...
void OnLeftDown(int x, int y) {
	...
	caret_region = RECT2Rect(D2D1::RectF(
		caret_metrics.left + 100,
		caret_metrics.top + 100,
		caret_metrics.left + 100 + 1,
		caret_metrics.top + 100 + caret_metrics.height));
	SetCaret(caret_region);
