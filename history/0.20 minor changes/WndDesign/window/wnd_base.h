...
AbstractInterface IWndBase{
	...
	const Rect GetRegionOnLayer() const { return Rect(ConvertToLayerPoint(point_zero), GetSize()); }
	...
	//// If you call BeginRegionUpdate(), all updating operations of a region will be combined to a union region,
	////   which will be updated when EndRegionUpdate() is called.
	//virtual void BeginRegionUpdate() pure;
	//virtual void EndRegionUpdate() pure;

	// *All region update operations will be combined in a message handling process.

	virtual void UpdateRegion(Rect region) const pure;
