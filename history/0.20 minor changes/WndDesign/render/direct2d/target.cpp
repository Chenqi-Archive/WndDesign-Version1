...
const Size Target::GetSize() const {
	return SIZE2Size(_target->GetSize());
}
...
TargetResourceManager::TargetResourceManager(const SystemWndLayer& layer, Handle hWnd) :
	_layer(layer),
	_hWnd(hWnd),
	_system_target(nullptr),
	_system_target_container(),
	_solid_color_brush(nullptr),
	_bitmap_brush(nullptr) {
	CreateResources();
}
...
void TargetResourceManager::CleanUnusedTargets() {
	for (auto it = _target_allocator.begin(); it != _target_allocator.end();) {
		it->second.CleanFreeTargets();
		if (it->second.IsEmpty()) {
			_target_allocator.erase(it++);
		} else {
			it++;
		}
	}
