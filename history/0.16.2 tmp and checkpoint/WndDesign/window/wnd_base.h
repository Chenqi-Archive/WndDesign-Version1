...
public:
	...
	virtual bool UpdateChildRegion(Ref<IWndBase*> child_wnd) pure;
	// Reset the child window's render style after the render style has changed.
	virtual void UpdateChildRenderStyle(Ref<IWndBase*> child_wnd) pure;
	...
	//   until UpdateRegion() is explicitly called, or parent window starts drawing the window.
