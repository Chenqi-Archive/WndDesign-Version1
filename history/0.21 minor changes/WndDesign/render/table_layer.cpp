...
void TableLayer::RedrawChild(Rect region_to_update) const {
	...
	for (uint index_column = first_grid.column + 1; index_column < GetParent()._columns.size(); ++index_column) {
		...
		figure_queue.append(
			new Rectangle(0, color_transparent, line_background),
	...
	if (IsRedirected()) {
		GetParent().RedirectLayerFigureQueue(figure_queue, region_to_update, vector_zero);  // No need to change coordinates.
	} else {
