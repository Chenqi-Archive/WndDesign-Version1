...
EditBox::EditBox(const ObjectBase& parent, const EditBoxStyle& style, wstring& text) :
	TextBox(parent, style, text),
...
void EditBox::AppendFigure(FigureQueue& figure_queue, Rect region_to_update) {
	// Draw caret.
	if (_caret_state == CaretState::Show || _caret_state == CaretState::BlinkShow) {
		figure_queue.append(new ColorFigure(_edit_style._caret_color), _caret_region);
	}
	// Draw selection.
	if (HasSelection()) {
		for (auto& it : _selection_info) {
			...
			figure_queue.append(new ColorFigure(_edit_style._selection_color), region);
		}
	}
}
...
void EditBox::Insert(wchar ch) {
	if (HasSelection()) {
		...
		_text.replace(_selection_begin, _selection_end - _selection_begin, 1, ch); 
		...
		_text.insert(_caret_text_position, 1, ch);
		(*this)->TextUpdated();  // Must update text layout before setting caret.
		SetCaret(_caret_text_position + 1, false);
	}
}
...
void EditBox::Insert(const wstring& str) {
	if (HasSelection()) {
		_text.replace(_selection_begin, _selection_end - _selection_begin, str);
		(*this)->TextUpdated();
		SetCaret(_selection_begin + static_cast<uint>(str.length()), false);
	} else {
		_text.insert(_caret_text_position, str);
		(*this)->TextUpdated();
...
void EditBox::Delete(bool is_backspace) { // Similar to Insert() above.
	if (HasSelection()) {
		...
		_text.erase(_selection_begin, _selection_end - _selection_begin); 
		(*this)->TextUpdated();  // Must update text layout before setting caret.
		SetCaret(_selection_begin, false);
	} else {
		...
		if (is_backspace) {
			if (_caret_text_position == 0) { 
				return; // At the beginning of the text string. Do nothing.
			}
			...
			_text.erase(_caret_text_position, character_length);
			(*this)->TextUpdated();
		} else {
			if (_caret_text_position >= _text.length()) {
				return;// At the end of the text string. Do nothing.
			}
...
void EditBox::ImeComposition(const wstring& composition_string) {
	_text.replace(_ime_composition_begin, _ime_composition_end - _ime_composition_begin, composition_string);
	(*this)->TextUpdated();  // Must update text layout before setting caret.
...
void EditBox::Copy() {
	if (HasSelection()) {
		SetClipboardData(_text.substr(_selection_begin, _selection_end - _selection_begin));
	}
}
