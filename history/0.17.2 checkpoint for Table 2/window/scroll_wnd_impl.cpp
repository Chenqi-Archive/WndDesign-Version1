...
WNDDESIGN_API unique_ptr<IScrollWnd, IWndBase::Deleter> 
IScrollWnd::Create(Ref<IWndBase*> parent, const StyleType& style, Ref<ObjectBase*> object) {
	unique_ptr<IScrollWnd, IWndBase::Deleter> wnd(new _Scroll_Wnd_Impl(new WndStyle(style), object, style.entire_region));
...
void _Scroll_Wnd_Impl::SizeChanged() {
	BlockComposition();  // There may be composition requests sent, ignore them. The window will be composited later.
