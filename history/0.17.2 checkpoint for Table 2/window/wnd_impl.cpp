...
WNDDESIGN_API unique_ptr<IWnd, IWndBase::Deleter> 
IWnd::Create(Ref<IWndBase*> parent, const WndStyle& style, Ref<ObjectBase*> object) {
	unique_ptr<IWnd, IWndBase::Deleter> wnd(new _Wnd_Impl(new WndStyle(style), object));
...
void _Wnd_Impl::Composite(Rect region_to_update) const {
	...
	if (!_base_layer.IsRedirected()) {
		figure_queue.Push(
			LayerFigure::Create(_base_layer, _base_layer.ConvertToLayerPoint(region_to_update.point)), 
			region_to_update
		);
	} else {
	...
	for (auto& it : _top_layers) {
		...
		figure_queue.Push(LayerFigure::Create(it, point_on_layer), intersected_region);
	}

	// Draw the border as a rectangle. Rounded rectangle may be supported later, which requires clipping.
	if (GetStyle().border._width > 0) {
		figure_queue.Push(
			Rectangle::Create(GetStyle().border._width, GetStyle().border._color, background_transparent),
