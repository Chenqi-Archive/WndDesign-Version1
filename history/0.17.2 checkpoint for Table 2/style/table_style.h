...
struct TableStyle : WndStyle {
	...
	// Determines if the table's size will automatically adjust when content changes, and determine if scrolling will 
	//   be enabled when content region overflows the window region.
...
#pragma message("To do: AutoResize may be moved to WndStyle.")
	struct AutoResize {
	public:
	...
	public:
		constexpr AutoResize& width_auto_resize() { _width_auto_resize = true; return *this; }
		constexpr AutoResize& height_auto_resize() { _height_auto_resize = true; return *this; }
		constexpr AutoResize& enable_scrolling() { _enable_scrolling = true; return *this; }
	}auto_resize;
