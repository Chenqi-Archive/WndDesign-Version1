...
struct WndStyle {
	...
	// Determines if the window's size will automatically adjust when content or child window changes.
	// The adjusted size will overwrite size_normal, and will still be bounded between size_min and size_max.
	// [For TextBox and Table]
	struct AutoResizeStyle {
	public:
		bool _width_auto_resize = false;
		bool _height_auto_resize = false;
		bool _enable_scrolling = false; // [For Table] If the content region exceeds the window size, 
										//   determines if the layer will be scrollable.
		bool _MayAutoResize() const { return _width_auto_resize || _height_auto_resize; }
		bool _NotAutoResize() const { return !_width_auto_resize && !_height_auto_resize; }
		bool _BothAutoResize() const { return _width_auto_resize && _height_auto_resize; }
		bool _WidthAutoResizeOnly() const { return _width_auto_resize && !_height_auto_resize; }
		bool _HeightAutoResizeOnly() const { return !_width_auto_resize && _height_auto_resize; }

	public:
		constexpr AutoResizeStyle& width_auto_resize(bool width_auto_resize = true) { _width_auto_resize = width_auto_resize; return *this; }
		constexpr AutoResizeStyle& height_auto_resize(bool height_auto_resize = true) { _height_auto_resize = height_auto_resize; return *this; }
		constexpr AutoResizeStyle& enable_scrolling() { _enable_scrolling = true; return *this; }
	}auto_resize;
	...
	public:
		uint _width = 0; 
		uint _radius = 0;	// Rounded angle radius.
		Color _color = ColorSet::Black;
	public:
		// supports px only.
		constexpr BorderStyle& width(ValueTag width) { _width = width.AsUnsigned(); return *this; }
		constexpr BorderStyle& radius(ValueTag radius) { _radius = radius.AsUnsigned(); return *this; }
		constexpr BorderStyle& color(Color color) { _color = color; return *this; }
	}border;


	// Background
	Background background;  // class Background is defined in background.h
							// background may contain Image resource.
};
