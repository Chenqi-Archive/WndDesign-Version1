...
public:
	...
	Rect GetCachedRegion() const { return _tile_cache.GetCachedRegion(); }

protected:
	// Get the visible region on layer, for refreshing cached region.
	// The visible region should be provided every time the layer's region has changed.(moving, resizing, etc.)
	//   But this was not implemented
	Rect GetLayerVisibleRegion() const;
...
public:
	...
	void EndRegionUpdate() {
		...
		RegionUpdated(_region_to_update, true);
...
public:
	...
	virtual Ref<_WndBase_Impl*> HitTestChild(Point point) const pure;
...
public:
	void SetResourceManager(Ref<TargetResourceManager*> resource_manager) {
		bool need_redraw = _tile_cache.SetResourceManager(resource_manager);
		SetChildResourceManager(resource_manager);
		// Update the layer if resource manager has changed.
		if (need_redraw) { RegionUpdated(region_infinite, false); }
	}
...
public:
	Ref<_WndBase_Impl*> HitTestChild(Point point) const override;
};
...
public:
	...
	// When the parent window's size has changed, reset the size of composite region, 
	//   and reset the accessible region accroding to entire_region style (mainly for scrollwnd),
	//   and current point will also be recalculated. 
	// Return true if accessible region has changed. (Useful for scrollwnd)
	bool ParentSizeChanged(Size parent_size);
	...
	////             Child Windows and Drawing             ////
	///////////////////////////////////////////////////////////
	// Spatial index for child windows.
private:
	struct WndObject { 
...
private:
	...
	// Retruns the region to update.
	const Rect ResetChildRegion(); // For ScrollWnd when accessible region is changed.
