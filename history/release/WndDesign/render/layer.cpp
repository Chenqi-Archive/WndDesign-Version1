...
bool MultipleWndLayer::ParentSizeChanged(Size parent_size) {
	...
	Rect old_cached_region = GetCachedRegion();

	bool tile_size_changed = _tile_cache.SetAccessibleRegion(new_region, GetLayerVisibleRegion());
	Rect region_to_update = ResetChildRegion();
	if(tile_size_changed){
		RegionUpdated(region_infinite, false);
	} else {
		RegionUpdated(region_to_update, false);  // Update modified child region.
		Rect new_cached_region = GetCachedRegion();
		_tile_cache.SetTileMask(old_cached_region.Intersect(new_cached_region));
		RegionUpdated(new_cached_region, false);  // Update new cached region.
		_tile_cache.ClearTileMask();
	}
...
const Rect MultipleWndLayer::ResetChildRegion() {
