...
// Use max_width to calculate max_text_width (minus padding and border), used for auto resizing.
inline uint CalculateMaxTextWidth(const TextBoxStyle& style, uint max_width) {
	...
	int border_width = static_cast<int>(style.border._width);
	int text_width = static_cast<uint>(max_width) - (padding._left.AsSigned() + padding._right.AsSigned() + border_width * 2);
	return text_width > 0 ? static_cast<uint>(text_width) : 0;
}

inline uint CalculateMaxTextHeight(const TextBoxStyle& style, uint max_height) {
	const TextBoxStyle::PaddingStyle& padding = style.padding;
	if (padding._top.IsPercent() || padding._down.IsPercent()) { ExceptionDialog(style_parse_exception); }
	int border_width = static_cast<int>(style.border._width);
	int text_height = static_cast<uint>(max_height) - (padding._top.AsSigned() + padding._down.AsSigned() + border_width * 2);
	return text_height > 0 ? static_cast<uint>(text_height) : 0;
}

// Use text width to calculate textbox width (add padding and border).
