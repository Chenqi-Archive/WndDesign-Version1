...
void _Scroll_Wnd_Impl::SizeChanged() {
	Point view_center_point = Rect(GetScrollPosition(), _base_layer.GetCompositeRegion().size).Center();

	// Recalculate the base layer's entire region.
	// If accessible region has changed, recalculate all child window's region, and redraw them on base layer.
	bool accessible_region_changed = _base_layer.ParentSizeChanged(GetSize());
	//if (accessible_region_changed) {  // Already done by base layer.
	//	_base_layer.ResetChildRegion(); 
	//}
	...
	Point current_view_center_point = Rect(GetScrollPosition(), _base_layer.GetCompositeRegion().size).Center();
	SetScrollPosition(view_center_point - (current_view_center_point - GetScrollPosition()));
}
...
void _Scroll_Wnd_Impl::SetEntrieRegion(ScrollWndStyle::RegionStyle entire_region) {
	_base_layer._entire_region = entire_region;
	_base_layer.ParentSizeChanged(GetSize()); // Recalculate accessible region.
	DispatchMessage(Msg::Scroll, nullptr);
	UpdateRegion(region_infinite);	// Composite.
}
